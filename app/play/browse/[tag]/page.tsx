...
export default async function PlayBrowseTagPage({
  params,
}: {
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: "640px", margin: "0 auto", marginTop: "3.5rem" }}>
        <p style={{ fontSize: "0.65rem", fontWeight: 500, letterSpacing: "0.14em", fontVariant: "small-caps", color: "rgba(10,10,10,0.45)", marginBottom: "0.5rem" }}>
          {category.title}
        </p>
        <h1 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 700, letterSpacing: "-0.02em", marginBottom: "0.75rem" }}>
          {tag}
        </h1>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {poems.map((poem) => (
            <Link
              key={poem.slug}
              href={`/play/browse/${encodeURIComponent(tag)}/${poem.slug}`}
              style={{
            ...
            >
              ...
              <span style={{ display: "block", fontSize: "0.75rem", color: "rgba(10,10,10,0.4)", marginTop: "0.2rem" }}>
                {poem.date}
              </span>
            </Link>
          ))}
